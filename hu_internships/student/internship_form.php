<?php
/**
 * student/internship_form.php — ฟอร์มยื่นคำขอฝึกงาน (Internship Request Form)
...
$edit_data = null;
if ($edit_id) {
    // โหลดข้อมูลเก่าที่อนุญาตให้แก้ (0 หรือ 9)
    $chk_edit = $conn->prepare("SELECT * FROM internship_requests WHERE request_id = ? AND student_id = ? AND status IN (0, 9)");
    $chk_edit->execute([$edit_id, $user_id]);
    $edit_data = $chk_edit->fetch(PDO::FETCH_ASSOC);
}

if ($edit_data) {
    $existing = false; // อนุญาตให้แสดงฟอร์ม
} else {
    // ตรวจสอบว่านิสิตเคยยื่นคำขอที่ไม่ได้ถูกยกเลิก/ให้แก้ไข (9) หรือไม่
    $chk = $conn->prepare("SELECT request_id, company_name, status FROM internship_requests WHERE student_id = ? AND status IN (0, 1, 2, 3, 4) LIMIT 1");
    $chk->execute([$user_id]);
    ...
    if (!$company_name) {
        ...
        try {
            if ($edit_data) {
                ...
                $stmt = $conn->prepare("
                    UPDATE internship_requests SET
                        company_name=?, company_type=?, company_address=?, company_province=?,
                        ...
                        status=0, request_date=CURRENT_TIMESTAMP
...
<div class="container" style="max-width:960px; padding:2rem 1.5rem;">
    ...
    <div style="margin-bottom:2rem;">
        <a href="../dashboard.php"
            style="display:inline-flex;align-items:center;gap:0.4rem;font-size:0.88rem;color:var(--primary-color);text-decoration:none;margin-bottom:0.75rem;opacity:0.8;">
        ...
        <h1 style="font-size:1.8rem;font-weight:800;color:var(--text-color);margin:0;">
            <?= $t['internship_form_title'] ?></h1>
    ...
    <?php if ($existing): ?>
        <!-- ========== เคยยื่นแล้ว ========== -->
        <div
            style="background:var(--card-bg);border-radius:12px;padding:2rem;border:1px solid var(--border-color);border-left:4px solid var(--warning-color);">
        ...
        <form method="POST" id="internshipForm" novalidate>
            ...
            <div class="iform-card">
                ...
                <div class="form-row">
                    <div class="form-group" style="flex:2;">
                        ...
                        <input type="text" id="company_name" name="company_name" class="form-control"
                            placeholder="<?= $lang === 'en' ? 'e.g. ABC Company Limited' : ' บริษัท เอบีซี จำกัด' ?>"
                            value="<?= htmlspecialchars($_POST['company_name'] ?? $edit_data['company_name'] ?? '') ?>"
                            required>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="company_type" name="company_type" class="form-control">
                            <?php foreach ([
                                ...
                                <option value="<?= $v ?>" <?= ($_POST['company_type'] ?? $edit_data['company_type'] ?? 'private') === $v ? 'selected' : '' ?>><?= $l ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="company_address" name="company_address" class="form-control" rows="2"
                        placeholder="Address details..."><?= htmlspecialchars($_POST['company_address'] ?? $edit_data['company_address'] ?? '') ?></textarea>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="company_province" name="company_province" class="form-control"
                            ...
                            value="<?= htmlspecialchars($_POST['company_province'] ?? $edit_data['company_province'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        ...
                        <input type="text" id="company_phone" name="company_phone" class="form-control"
                            ...
                            value="<?= htmlspecialchars($_POST['company_phone'] ?? $edit_data['company_phone'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        ...
                        <input type="url" id="company_website" name="company_website" class="form-control"
                            ...
                            value="<?= htmlspecialchars($_POST['company_website'] ?? $edit_data['company_website'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="department" name="department" class="form-control"
                        ...
                        value="<?= htmlspecialchars($_POST['department'] ?? $edit_data['department'] ?? '') ?>">
                </div>
            </div>
                ...
                <div class="form-row">
                    <div class="form-group" style="flex:2;">
                        ...
                        <input type="text" id="contact_person" name="contact_person" class="form-control"
                            ...
                            value="<?= htmlspecialchars($_POST['contact_person'] ?? $edit_data['contact_person'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        ...
                        <input type="text" id="contact_position" name="contact_position" class="form-control"
                            ...
                            value="<?= htmlspecialchars($_POST['contact_position'] ?? $edit_data['contact_position'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="contact_phone" name="contact_phone" class="form-control"
                            ...
                            value="<?= htmlspecialchars($_POST['contact_phone'] ?? $edit_data['contact_phone'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label for="contact_email"><?= $t['contact_email_label'] ?></label>
                        <input type="email" id="contact_email" name="contact_email" class="form-control"
                            placeholder="supervisor@company.com"
                            value="<?= htmlspecialchars($_POST['contact_email'] ?? $edit_data['contact_email'] ?? '') ?>">
                    </div>
                </div>
            </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="date" id="internship_start" name="internship_start" class="form-control"
                            value="<?= htmlspecialchars($_POST['internship_start'] ?? $edit_data['internship_start'] ?? '') ?>"
                            min="<?= $edit_data ? '' : date('Y-m-d') ?>" required>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="date" id="internship_end" name="internship_end" class="form-control"
                            value="<?= htmlspecialchars($_POST['internship_end'] ?? $edit_data['internship_end'] ?? '') ?>"
                            required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="work_days" name="work_days" class="form-control">
                            <?php foreach ([
                                ...
                                <option value="<?= $v ?>" <?= ($_POST['work_days'] ?? $edit_data['work_days'] ?? 'จันทร์-ศุกร์') === $v ? 'selected' : '' ?>><?= $l ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="text" id="work_hours" name="work_hours" class="form-control"
                            ...
                            value="<?= htmlspecialchars($_POST['work_hours'] ?? $edit_data['work_hours'] ?? '09:00-18:00') ?>">
                    </div>
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    <select id="advisor_id" name="advisor_id" class="form-control">
                        ...
                        <?php foreach ($teachers as $tc): ?>
                            <option value="<?= htmlspecialchars($tc['username']) ?>" <?= ($_POST['advisor_id'] ?? $edit_data['advisor_id'] ?? '') === $tc['username'] ? 'selected' : '' ?>>
            ...
            <div style="display:flex;justify-content:flex-end;gap:1rem;margin-top:0.5rem;">
                ...
                <button type="submit" class="btn btn-primary"
                    style="padding:0.85rem 2.5rem;font-size:1rem;font-weight:700;display:inline-flex;align-items:center;gap:0.5rem;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2.5">
                        <polyline points="20 6 9 17 4 12" />
                    </svg>
                    <?= $edit_data ? 'บันทึกการแก้ไข' : 'ยื่นคำขอฝึกงาน' ?>
                </button>
            </div>
        </form>
